...
class IntProgress(Widget):
    ...
    def __initViews__(self):
        ...
        self.widget = widgets.IntProgress(description=self.desc,value = self.value, min = self.min, max = self.max, bar_style = self.bar_style,
        orientation= self.orientation)
        #2nd Create Represent Button
        self.represent = widgets.Button(description= "Button - "+ str(self.id),disabled=False,button_style='')
        self.represent.description = "IntProgress - "+ str(self.id)
